...
#Before running, please pip install openai python-docx python-dotenv fastapi uvicorn PdfReader PyPDF2
#To run, run the code and in terminal "uvicorn api:app --reload", then use the local server link
...
class DocumentAnalyzer:
    ...
    def calculate_risk_score(self, analyzed_clauses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        max_possible_score = clause_count * 3 * 2.0 
...
async def run_diagnostics():
    try:
        dependencies = ["openai", "docx", "dotenv", "fastapi", "PyPDF2"]
        missing_deps = [dep for dep in dependencies if dep not in globals()]
        if missing_deps:
            raise ImportError(f"Missing dependencies: {', '.join(missing_deps)}")
        
        required_env_vars = ["API_BASE_URL", "API_KEY", "MODEL_ID"]
        missing_env_vars = [var for var in required_env_vars if not os.getenv(var)]
        if missing_env_vars:
            raise EnvironmentError(f"Missing environment variables: {', '.join(missing_env_vars)}")
        if not os.path.exists(templates.directory):
            raise FileNotFoundError(f"Templates directory not found: {templates.directory}")
        if not isinstance(app, FastAPI):
            raise RuntimeError("FastAPI app instance is not configured correctly.")
        return {"status": "success", "message": "All diagnostics passed successfully!"}
...
@app.post("/api/prompt")
async def handle_prompt(payload: Dict[str, str]):
    """Handle the prompt selection from the frontend."""
    action = payload.get("action")
    if not action:
        raise HTTPException(status_code=400, detail="No action provided.")
    if action == "analyze_contract":
        response = "Analyzing the contract. Please upload a file."
    elif action == "summarize_document":
        response = "Summarizing the document. Upload the file to begin."
    elif action == "identify_risks":
        response = "Identifying risks in the document. Upload your file."
    elif action == "extract_key_information":
        response = "Extracting key information from the document. Upload your file."
    else:
...
async def analyze(file: UploadFile = File(...)) -> HTMLResponse:
    ...
    try:
        ...
        result_html = f"""
            <h1 class="centered-heading">Analysis Results</h1>
            <p class="risk_score"><b>Overall Risk Score:</b> {analysis_result['overall_score']}%</p>
            <ul>
        """
